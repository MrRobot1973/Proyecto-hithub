...
def generarMenu(usuario,):
    ...
    with st.sidebar:
        ...
        if nombre == "admin":
            ...
            st.write(f"Rol: Administrador")

            # Mostramos los enlaces de páginas
            st.page_link("inicio.py", label="Inicio", icon=":material/home:")
            st.page_link("pages/pagina1.py", label="Amigos", icon=":material/group:")
            st.page_link("pages/pagina2.py", label="Conversaciones", icon=":material/comic_bubble:")
            st.page_link("pages/pagina3.py", label="Configuración", icon=":material/settings:")    
        ...
        if nombre == "Analista Ventas" :
            st.write(f"Hola **:blue-background[{nombre}]** ")
            st.write(f"Rol: Usuario")
            # Mostramos los enlaces de páginas
